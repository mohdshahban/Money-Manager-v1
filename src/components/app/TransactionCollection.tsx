                ...
                return (
                  <li key={tx.id} className="group flex items-start gap-3 rounded-xl px-2 py-3 transition-colors hover:bg-muted/50">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 line-clamp-2 text-xs text-muted-foreground">{categoryLabel} · {acc?.name ?? "—"}{tx.payment_method ? ` · ${tx.payment_method}` : ""}{project ? ` · ${project.name}` : ""} · {format(new Date(tx.occurred_at), "HH:mm")}</p>
                      <ReceiptIndicator receiptPath={tx.receipt_path} />
                      {(tx.tags?.length ?? 0) > 0 && <div className="mt-1.5 flex flex-wrap gap-1">{tx.tags!.map((tag) => onTagClick ? <button key={tag} type="button" onClick={() => onTagClick(tag)} className="rounded-full border border-primary/25 bg-primary/5 px-2 py-0.5 text-[10px] font-medium text-primary hover:bg-primary/10">#{tag}</button> : <span key={tag} className="rounded-full border px-2 py-0.5 text-[10px] text-muted-foreground">#{tag}</span>)}</div>}
                    </div>
            ...
            {transactions.map((tx) => {
              ...
              return (
                <tr key={tx.id} className="group transition-colors hover:bg-muted/40">
                  ...
                  <td className="max-w-[340px] px-4 py-3 align-top"><p className="line-clamp-2 font-medium">{tx.vendor || tx.notes || cat?.name || "Transaction"}</p>{(tx.tags?.length ?? 0) > 0 && <div className="mt-1 flex flex-wrap gap-1">{tx.tags!.slice(0, 3).map((tag) => onTagClick ? <button key={tag} type="button" onClick={() => onTagClick(tag)} className="text-[10px] font-medium text-primary hover:underline">#{tag}</button> : <span key={tag} className="text-[10px] text-muted-foreground">#{tag}</span>)}</div>}</td>
